...
story = """At a great meeting of the Animals, who had gathered to elect a new ruler, the Monkey was asked to dance. This he did so well, with a thousand funny capers and grimaces, that the Animals were carried entirely off their feet with enthusiasm, and then and there, elected him their king.
...
length = 0
word_position = 0
for word in enumerate(story):
    if (word[1] == ' ' or word[1] == '.') and (word[1] == ' ' and story[word_position - 1] != '.'):
        first_letter = story[word_position:word[0]]
        # print(first_word[0])
        word_position = word[0] + 1
        remaining_word = first_letter[1:len(first_letter)]
        # print(remaining_word)
        edit_story += remaining_word + first_letter[0] + last_letter + " "
